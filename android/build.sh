#!/bin/bash
# Build EQ Player APK for the FUDONI GC888 projector (Android 9 / API 28)
set -euo pipefail
...
PKG="com.mediacast.eqplayer"
OUT="$SCRIPT_DIR/build"
APK_UNSIGNED="$OUT/eqplayer-unsigned.apk"
APK_ALIGNED="$OUT/eqplayer-aligned.apk"
APK_SIGNED="$OUT/eqplayer.apk"
KEYSTORE="$SCRIPT_DIR/debug.jks"

echo "=== EQ Player Build ==="
...
# 2. Link manifest to create base APK (no compiled resources needed — we use no res/)
...
# Include generated R.java if it exists
if [ -f "$OUT/com/mediacast/eqplayer/R.java" ]; then
    echo "$OUT/com/mediacast/eqplayer/R.java" >> "$OUT/sources.txt"
fi
javac \
